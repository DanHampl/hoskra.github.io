...
function draw() {
    ...
    cl(bass + " " + mid + " " + high);
    bass = Math.abs(bass) / 50;
    mid = Math.abs(mid) / 10;
    high = Math.abs(high) / 5;
    ...
    bass = bass < MIN_BASS ? MIN_BASS : bass;
    mid = mid < MIN_MID ? MIN_MID : mid;
    high = high < MIN_HIGH ? MIN_HIGH : high;

    vol = Mic.getVolume() * 40;
    cl(bass + " " + mid + " " + high);

    // cl(vol);
    let camZ = camera.position.z;

// can get error getInverse() can't invert matrix, determinant is 0

    bassObjects.forEach(x => {
        x.scale.set(bass, bass, bass)
        x.position.set( -100, 0, -3000 + camZ);
    });
    midObjects.forEach((x, index) => {
        x.scale.set(mid, mid, mid)
    });
    highObjects.forEach((x, index) => {
        x.scale.set(high, high, high)
    });

    volumeObject.scale.set(vol, 1, 1)
    volumeObject.position.set( -50, -2000, 0);

    midObjects[0].position.set( 600, 0, -1500 + camZ);
    midObjects[1].position.set( -800, 0, -1500 + camZ);
    highObjects[0].position.set( -200, 0, -1000 + camZ);
    highObjects[1].position.set( 600, 0, -500 + camZ);
    highObjects[2].position.set( -600, 0, -400 + camZ);
    }
}
...
function createObjects() {
    if (!NO_SHAPES) {
    // volume
    volumeObject = new THREE.Mesh( new THREE.BoxBufferGeometry( 0, 0, 100000 ), volumeMaterial );
    volumeObject.position.set( -100, 0, 0 );
    scene.add( volumeObject );

    // center red
    object = new THREE.Mesh( new THREE.SphereBufferGeometry( 10 ), redMaterial );
    object.position.set( -100, 200, -2000 );
    scene.add( object );
    bassObjects.push(object);

    // right
    object = new THREE.Mesh( new THREE.SphereBufferGeometry( 10 ), blueMaterial );
    ...
    // high
    object = new THREE.Mesh( new THREE.SphereBufferGeometry( 10 ), whiteMaterial );
